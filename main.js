...
async function openVideoAndFetchDetails(videoPath, episodeId) {
    ...
    try {
        ...
        if (subtitleTracks.length > 0) {
            ...
            subtitlePath = await extractSubtitles(videoPath, selectedTrack.id, subPath);
        }
        ...
        if (existingResult) {
            ...
            createVideoWindow(videoPath, newTitle, episodeId, subtitlePath);
        } else {
            console.log("未找到存储结果，开始识别:", videoPath);
            const response = await recognizeVideo(videoPath);
            console.log("post结果:", response.isMatched, response.animeTitle);
            if (response.isMatched) {
                ...
                createSelectionWindow(response.matches, videoPath, episodeId, subtitlePath);
            }
        }
    } catch (error) {
...
function parseSubtitleTracks(stderrData) {
    ...
    while ((match = streamPattern.exec(stderrData)) !== null) {
        const [_, id, handlerName, title] = match;
        const subtitleTrack = {
            id: id
        };
        if (handlerName) {
...
function extractSubtitles(videoPath, trackId, outputDir) {
    const videoName = path.basename(videoPath, path.extname(videoPath));
    const outputPath = path.join(subPath, `${videoName}.srt`);
    console.log('ffmpeg sub:',outputPath)
    const ffmpegArgs = ['-i', videoPath, '-map', trackId, '-c:s', 'srt','-y', outputPath];
    const isMac = process.platform === 'darwin';
    const ffmpegPath = isMac ? './ffmpeg/ffmpeg_mac' : './ffmpeg/ffmpeg_win/bin/ffmpeg.exe';
    return new Promise((resolve, reject) => {
